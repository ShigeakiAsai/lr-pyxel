...
以下は、より限定的で優先度の低い制限として、v1.0の制限事項として
明記しています：

- トップレベルのバンクリスト（`colors`/`channels`/`tones`/`images`/
  `sounds`/`tilemaps`/`musics`）では、スライスによるアクセス
  （例：`pyxel.colors[1:3]`）に対応しておらず、単一インデックスの
  みサポートしています。`Music.seqs`だけは例外的にスライス代入に
  対応しています（本家自身のテストがそれを明示的に要求しているため）。
...
- 一部のエラーメッセージは、本家と文言が完全には一致していません
  （例：`PyO3`自体のバージョンに依存する自動生成メッセージが1件）。
  いずれも例外の種類自体は正しく送出されており、機能的な問題は
  ありません。単に文言が一字一句同じではない、という程度の差です。
